...
参数值的读取见 §3.1（用 `wing.extract.Path` 提取器）。
...
handler 签名是 `fn(*Ctx, ...提取器) !返回类型`。提取器参数由框架在编译期识别并填充：路径参数、查询参数、请求头、Cookie（`wing.extract.Cookies(T)`，用法同 Query），以及请求体（JSON、表单、原始字节、multipart 文件上传）。

### 3.1 路径参数：`wing.extract.Path(T)`
...
### 3.3 请求体：`wing.extract.Json(T)`

`wing.extract.Json(T)` 消费并解析 JSON 请求体（响应侧的 JSON 序列化是 `wing.respond.Json(T)`，见 §5——同名、方向相反）。**它必须是 handler 的最后一个参数**（body 提取器每 handler 至多一个且居末，否则编译报错）。
...
fn createUser(
    ctx: *Ctx,
    db: *Db,                       // 状态投影，见 §4
    body: wing.extract.Json(CreateUserReq), // body 提取器，必须居末
...
请求的 `Content-Type` 必须是 `application/json`（或 RFC 6839 的 `application/*+json`），否则返回 415；JSON 解析失败 → 400（均经 recover 中间件映射）。

> 内存提示：解析后的数据存活在请求 arena（`ctx.arena`），请求结束自动回收。需要更长生命周期请自行拷贝到你的 State。

### 3.4 表单：`wing.extract.Form(T)`

解析 `application/x-www-form-urlencoded` 请求体，字段规则与 `Query` 完全一致（同一解析器）。同为 body 提取器：每 handler 至多一个且居末。

```zig
const LoginForm = struct { user: []const u8, remember: bool = false };

fn login(ctx: *Ctx, form: wing.extract.Form(LoginForm)) anyerror![]const u8 {
    _ = ctx;
    return if (form.value.remember) "welcome back" else "welcome";
}
```

`Content-Type` 不是表单类型 → 415；缺必填字段 / 字段解析失败 → 400。文件上传（`multipart/form-data`）请用 `wing.extract.Multipart`（见 §3.7）。

### 3.5 请求头：`wing.extract.Headers(T)`

按字段名绑定请求头，字段名中的 `_` 映射为头名的 `-`（`user_agent` → `user-agent`，查找不区分大小写）。字段类型与必填/可选规则同 `Query`；字符串字段零拷贝借用头缓冲。

```zig
const ClientInfo = struct {
    x_client_version: u32,        // 必填，缺失 → 400
    user_agent: ?[]const u8,      // 可选，缺失 → null
};

fn info(ctx: *Ctx, h: wing.extract.Headers(ClientInfo)) anyerror![]const u8 {
    _ = ctx;
    _ = h.value.x_client_version;
    return "ok";
}
```

### 3.6 原始请求体：`wing.extract.Bytes`

把整个请求体原样收集到请求 arena，交给 handler 自行处理（自定义格式、代理转发等）。不校验 `Content-Type`；大小受服务端 `max_body_size` 约束（超限 → 413）。

```zig
fn ingest(ctx: *Ctx, raw: wing.extract.Bytes) anyerror![]const u8 {
    return std.fmt.allocPrint(ctx.arena, "received {d} bytes", .{raw.value.len});
}
```

### 3.7 文件上传：`wing.extract.Multipart`

解析 `multipart/form-data` 请求体（RFC 7578）。缓冲式：整个 body 收进请求 arena（受 `max_body_size` 约束，默认 16MiB，超限 → 413），各 part 的 `data` 零拷贝借用该缓冲。

```zig
fn upload(ctx: *Ctx, mp: wing.extract.Multipart) anyerror![]const u8 {
    const f = mp.file("file") orelse return error.InvalidMultipartBody; // → 400
    // f.filename / f.content_type / f.data
    return std.fmt.allocPrint(ctx.arena, "got {s} ({d} bytes), note={s}", .{
        f.filename.?, f.data.len, mp.field("note") orelse "<none>",
    });
}
```

```bash
curl -F file=@photo.jpg -F note=hi http://127.0.0.1:8080/upload
```

- `mp.field(name)`：首个同名文本字段的值；`mp.file(name)`：首个同名文件 part；`mp.parts`：全部 part。
- `Content-Type` 不是 `multipart/form-data` → 415；缺 boundary / 帧格式错 / part 缺 `name` → 400。
- 落盘由 handler 自己决定（框架不写临时文件）；需要流式大文件上传的场景暂不支持。
- 安全提醒：`filename` 是客户端原样值（可能含 `../`、绝对路径、任意字节）。落盘前必须自行清洗，不可直接拼接到路径里。
...
| `wing.respond.Json(T)` | 200，JSON 序列化，`application/json` |
| `wing.respond.Created(T)` | 201，JSON body + 可选 `Location` 头 |
| `wing.respond.Redirect` | 重定向（默认 302 Found，可改） |
| 自定义带 `toResponse(self, ctx)` 的类型 | 你定义 |

```zig
// JSON
fn getUser(ctx: *Ctx, path: wing.extract.Path(struct { id: u64 })) anyerror!wing.respond.Json(User) {
    _ = ctx;
    return .{ .value = .{ .id = path.value.id, .name = "ada" } };
}

// 201 Created + Location
fn createUser(ctx: *Ctx, body: wing.extract.Json(CreateUserReq)) anyerror!wing.respond.Created(User) {
...
//   head 提取器： wing.extract.Path(T) / wing.extract.Query(T) / wing.extract.Headers(T) / wing.extract.Cookies(T) / *StateField
//   body 提取器（至多一个且居末）： wing.extract.Json(T) / wing.extract.Form(T) / wing.extract.Bytes / wing.extract.Multipart
//   返回：   []const u8 / void / wing.respond.Json(T) / wing.respond.Created(T) / wing.respond.Redirect / 自定义 toResponse
